...
import { SEED_PROGRAMS } from '@/lib/programsSeed';
import { Sparkles } from 'lucide-react';

type Program = { id: string; name: string; description: string; startDate: string; status: string; category?: string; kind?: string; eventDate?: string; time?: string; location?: string; seats?: string; price?: string; badge?: string; ctaHref?: string; ctaLabel?: string; createdAt: any };

const EMPTY_PROG = { name: '', description: '', startDate: '', status: 'upcoming', category: 'Learning Hub', kind: 'program', eventDate: '', time: '', location: '', seats: '', price: '', badge: '', ctaHref: '', ctaLabel: '' };
...
export default function ProgramsPage() {
  ...
  const [newProg, setNewProg] = useState({ ...EMPTY_PROG });
  ...
  const seedInitialData = async () => {
    const ok = await confirm({
      tone: 'info',
      title: 'Seed initial programs & events?',
      itemName: `${SEED_PROGRAMS.length} entries`,
      description: 'Adds the default Summer Coding Bootcamp, hackathon, open day, and past programs to Firestore. Skips any name that already exists.',
    ...
    for (const seed of SEED_PROGRAMS) {
      ...
      await addDoc(collection(db, 'programs'), { ...seed, createdAt: serverTimestamp() });
      added += 1;
    }
    setSeeding(false);
    showToast('success', added === 0 ? 'No new entries to add: all seed items already exist.' : `Seeded ${added} entr${added === 1 ? 'y' : 'ies'}.`);
  };
  ...
  // Program CRUD
  const closeModal = () => { setShowCreate(false); setEditingProg(null); setNewProg({ ...EMPTY_PROG }); setShowOptional(false); };
  const createOrUpdateProgram = async () => {
    ...
    const payload = {
      ...
      kind: newProg.kind,
      eventDate: newProg.eventDate,
      time: newProg.time,
      location: newProg.location,
      seats: newProg.seats,
      price: newProg.price,
      badge: newProg.badge,
      ctaHref: newProg.ctaHref,
      ctaLabel: newProg.ctaLabel,
    };
    if (editingProg) await updateDoc(doc(db, 'programs', editingProg.id), payload);
    else await addDoc(collection(db, 'programs'), { ...payload, createdAt: serverTimestamp() });
  ...
  const startEdit = (p: Program) => {
    ...
    setNewProg({
      ...
      category: p.category || 'Learning Hub',
      kind: p.kind || 'program',
    ...
    // Auto-expand if any optional field is already filled, so post-event edits aren't hidden.
    const hasOptional = !!(p.eventDate || p.time || p.location || p.seats || p.price || p.badge || p.ctaHref || p.ctaLabel);
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          {isAdmin && (
            <button onClick={seedInitialData} disabled={seeding}
              className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 text-xs font-bold hover:border-amber-300 hover:text-amber-600 disabled:opacity-50 transition-all">
              ...
              <motion.div drag dragMomentum={false} dragElastic={0.1} initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
                ...
                <div className="px-6 pb-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-bold text-slate-700 mb-1.5 block">Type</label><select value={newProg.kind} onChange={(e) => setNewProg({ ...newProg, kind: e.target.value })} className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-brand-400 bg-white">{KINDS.map((k) => <option key={k.value} value={k.value}>{k.label}</option>)}</select></div>
                    <div><label className="text-xs font-bold text-slate-700 mb-1.5 block">Category</label><select value={newProg.category} onChange={(e) => setNewProg({ ...newProg, category: e.target.value })} className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-brand-400 bg-white">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  {/* ── Optional display details (event-style fields, also editable post-event) ── */}
                    ...
                    <AnimatePresence>{showOptional && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="px-4 pb-4 pt-1 space-y-3">
                          <p className="text-[11px] text-slate-400 leading-relaxed">Used by the public events listing. All optional — you can add or update them after the event has passed (e.g. recap link, attendance, photo gallery URL).</p>
                          <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Display date</label><input value={newProg.eventDate} onChange={(e) => setNewProg({ ...newProg, eventDate: e.target.value })} placeholder="e.g. June 14, 2026" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Time</label><input value={newProg.time} onChange={(e) => setNewProg({ ...newProg, time: e.target.value })} placeholder="e.g. 10:00 AM – 2:00 PM WAT" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                          </div>
                          <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Location</label><input value={newProg.location} onChange={(e) => setNewProg({ ...newProg, location: e.target.value })} placeholder="e.g. Abuja, Nigeria + Livestream" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                          <div className="grid grid-cols-3 gap-3">
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Seats</label><input value={newProg.seats} onChange={(e) => setNewProg({ ...newProg, seats: e.target.value })} placeholder="300 seats" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Price</label><input value={newProg.price} onChange={(e) => setNewProg({ ...newProg, price: e.target.value })} placeholder="Free / $29" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">Badge</label><input value={newProg.badge} onChange={(e) => setNewProg({ ...newProg, badge: e.target.value })} placeholder="$5K prizes / Past event" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">CTA link</label><input value={newProg.ctaHref} onChange={(e) => setNewProg({ ...newProg, ctaHref: e.target.value })} placeholder="/summer-coding-2026 or https://…" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                            <div><label className="text-[11px] font-bold text-slate-600 mb-1 block">CTA label</label><input value={newProg.ctaLabel} onChange={(e) => setNewProg({ ...newProg, ctaLabel: e.target.value })} placeholder="Register / Watch recap" className="w-full px-3 py-2.5 rounded-lg border-2 border-slate-200 text-xs focus:outline-none focus:border-brand-400" /></div>
                          </div>
                        </div>
                      </motion.div>
                    )}</AnimatePresence>
      ...
      <div className="space-y-4 mb-10">
        {(() => {
          const filteredPrograms = programs.filter((p) => {
            if (filterKind !== 'all' && (p.kind || 'program') !== filterKind) return false;
            if (filterStatus !== 'all' && (p.status || 'upcoming') !== filterStatus) return false;
            if (filterCategory !== 'all' && (p.category || '') !== filterCategory) return false;
            return true;
          });
          return filteredPrograms.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center"><FolderKanban className="w-10 h-10 mx-auto text-slate-200 mb-3" /><p className="text-slate-400 text-sm">{programs.length === 0 ? 'No programs yet.' : 'No items match the current filters.'}</p></div>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${prog.status === 'active' ? 'bg-emerald-50 text-emerald-600' : prog.status === 'completed' ? 'bg-slate-100 text-slate-500' : 'bg-brand-50 text-brand-600'}`}>{prog.status === 'completed' ? 'past' : prog.status}</span>
                      {prog.kind === 'event' && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-50 text-purple-600">Event</span>}
                      {prog.category && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700">{prog.category}</span>}
                    </div>
