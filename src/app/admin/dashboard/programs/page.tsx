...
export default function ProgramsPage() {
  ...
  const deleteProgram = async (id: string) => {
    ...
    const ok = await confirm({
      tone: 'danger',
      title: 'Delete course?',
      itemName: p?.name || 'Course',
      description: enrolled > 0
        ? `${enrolled} enrolled student${enrolled === 1 ? '' : 's'} will be unassigned. Type the course name to confirm.`
    ...
    showToast('success', 'Course deleted. Students unassigned.');
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Course</h2>
          <p className="text-slate-400 text-sm mt-1">Manage courses, students, and weekly office hours.</p>
        </div>
        {isAdmin && <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl gradient-bg text-white text-sm font-bold shadow-md hover:-translate-y-0.5 transition-all"><Plus className="w-4 h-4" /> New Course</button>}
      </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
              <motion.div drag dragMomentum={false} dragElastic={0.1} initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
                ...
                <div className="flex items-center justify-between px-6 pt-5 pb-3 cursor-grab active:cursor-grabbing select-none">
                  <div className="flex items-center gap-3"><div className="w-1 h-5 rounded-full bg-slate-200" /><h3 className="text-lg font-bold text-slate-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{editingProg ? 'Edit Course' : 'Create Course'}</h3></div>
                ...
                <div className="px-6 pb-6 space-y-4">
                  <div><label className="text-xs font-bold text-slate-700 mb-1.5 block">Course Name *</label><input value={newProg.name} onChange={(e) => setNewProg({ ...newProg, name: e.target.value })} placeholder="e.g. AI Developer Track Cohort 3" className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-brand-400" /></div>
                  ...
                  <button onClick={createOrUpdateProgram} disabled={creating || !newProg.name} className="w-full py-3.5 rounded-xl gradient-bg text-white text-sm font-bold shadow-md hover:-translate-y-0.5 disabled:opacity-60 transition-all flex items-center justify-center gap-2">
                    {creating ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{editingProg ? 'Save Changes' : 'Create Course'} <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </div>
              </motion.div>
            </div>
      ...
      {orphanedNames.length > 0 && (
        <div className="mb-6 space-y-3">
          <p className="text-xs font-bold uppercase tracking-wider text-amber-600 flex items-center gap-1.5"><AlertCircle className="w-3.5 h-3.5" /> Students assigned to missing courses ({orphanedNames.length})</p>
            ...
            <div key={name} className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-2">
                  {isAdmin && <button onClick={() => createFromOrphan(name)} className="px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-xs font-bold hover:-translate-y-0.5 transition-all flex items-center gap-1"><Plus className="w-3 h-3" /> Create Course</button>}
      ...
      <div className="space-y-4 mb-10">
        {programs.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center"><FolderKanban className="w-10 h-10 mx-auto text-slate-200 mb-3" /><p className="text-slate-400 text-sm">No courses yet.</p></div>
        ) : programs.map((prog, i) => {
